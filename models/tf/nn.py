...
class FCNet(Model):
    def __init__(self, input_shape: tuple, random_seed: int = 0):
    ...
    def _build_model(self, **kwargs):
        ...
        if optimizer == "adam":
            ...
            raise NotImplementedError(
                f'Optimizer "{optimizer}" has not been implemented yet'
            )
    ...
    def _train(
        ...
        if (
            self._learning_rate_decay_epochs > 0
            and self._learning_rate_decay_factor > 0.0
        ):
            callbacks.append(
