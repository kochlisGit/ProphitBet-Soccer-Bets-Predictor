...
class FCNetTuner(Tuner):
    ...
    def _create_model(self, trial) -> Model:
        ...
        model = FCNet(input_shape=self.x_train.shape[1:], random_seed=self.random_seed)

        noise_range = trial.suggest_float("noise_range", low=0.0, high=0.4)

        num_hidden_layers = trial.suggest_int(
            "num_hidden_layers", low=self._min_layers, high=self._max_layers, step=1
        )
        hidden_units_list = list(
            range(self._min_units, self._max_units + 1, self._units_increment)
        )
        hidden_layers = [
            trial.suggest_categorical(f"layer_{i}", hidden_units_list)
            for i in range(num_hidden_layers)
        ]
        batch_normalizations = [
            trial.suggest_categorical(f"bn_{i}", [False, True])
            for i in range(num_hidden_layers)
        ]
        activations = [
            trial.suggest_categorical(f"activation_{i}", [None, "tanh", "relu", "gelu"])
            for i in range(num_hidden_layers)
        ]
        regularizations = [
            trial.suggest_categorical(f"regularization_{i}", [None, "l1", "l2"])
            for i in range(num_hidden_layers)
        ]
        dropouts = [
