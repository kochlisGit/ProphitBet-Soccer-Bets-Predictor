...
class CorrelationsAnalyzerWindow(QDialog):
    """ Correlation analyzer window using logistic regression. """

    def __init__(self, df: pd.DataFrame):
        super().__init__()

        self._analyzer = CorrelationAnalyzer(df=df)

        self._seasons = self._analyzer.seasons
        self._methods = self._analyzer.methods
        self._feature_types = {'Home Team': 'home', 'Away Team': 'away'}
        self._colormap_dict = self._analyzer.colormap_dict

        self._title = 'Correlation Analysis'
        self._width = 350
        self._height = 190

        # Declare UI placeholders.
        self._combo_season = None
        self._combo_method = None
