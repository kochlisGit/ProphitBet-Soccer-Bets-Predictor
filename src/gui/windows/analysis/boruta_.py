...
from src.analysis import BorutaAnalyzer
from src.gui.widgets.plot import PlotWindow
from src.preprocessing.utils.target import TargetType


class BorutaAnalyzerWindow(QDialog):
    """ Boruta analyzer window. """

    def __init__(self, df: pd.DataFrame):
        super().__init__()

        self._analyzer = BorutaAnalyzer(df=df)
    ...
    def _analyze(self):
        ...
        PlotWindow(ax=ax, parent=self, title='Boruta Analysis').show()
