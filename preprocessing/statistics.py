...
class StatisticsEngine:
    ...
    def __init__(
        self, matches_df: pd.DataFrame, last_n_matches: int, goal_diff_margin: int
    ):
    ...
    def compute_statistics(self, statistic_columns: list) -> pd.DataFrame:
        ...
        matches_df[statistic_columns] = matches_df[statistic_columns].astype(
            dtype=np.int32
        )
        return matches_df
    ...
    def _compute_cumulated_home_wins(self) -> pd.Series:
        return self._compute_last_results(
            team_index=1, target_result_value="H", all_matches=True
        )

    def _compute_cumulated_home_losses(self) -> pd.Series:
        return self._compute_last_results(
            team_index=1, target_result_value="A", all_matches=True
        )

    def _compute_cumulated_away_wins(self) -> pd.Series:
        return self._compute_last_results(
            team_index=2, target_result_value="A", all_matches=True
        )

    def _compute_cumulated_away_losses(self) -> pd.Series:
        return self._compute_last_results(
            team_index=2, target_result_value="H", all_matches=True
        )

    def _compute_cumulated_home_draws(self) -> pd.Series:
        return self._compute_last_results(
            team_index=1, target_result_value="D", all_matches=True
        )

    def _compute_cumulated_away_draws(self) -> pd.Series:
        return self._compute_last_results(
            team_index=2, target_result_value="D", all_matches=True
        )

    def _compute_last_results(
        self, team_index: int, target_result_value: str, all_matches: bool = False
    ):
        last_results = []
            ...
            for i, match in enumerate(match_history):
                ...
                for previous_match in match_history[i + 1 :]:
                    if previous_match[team_index] == team_name:
                        ...
                        if (
                            target_higher_goals - target_lower_goals
                            >= self._goal_diff_margin
                        ):
    ...
    def _compute_total_results_rate(self, team_index: int, target_result_value: str):
        ...
        for season in range(self._max_season, self._min_season - 1, -1):
            ...
            for i, match in enumerate(match_history):
                ...
                last_result_rates.append(
                    np.nan
                    if total_results == 0
                    else round(target_results * 100 / total_results)
                )
    ...
    def _compute_last_cards(
        self, team_index: int, card_index: int, all_matches: bool = False
    ):
        last_cards = []
