...
def preprocess_training_dataframe(
    matches_df: pd.DataFrame, one_hot: bool
) -> (np.ndarray, np.ndarray):
    inputs = matches_df.dropna().drop(
        columns=["Season", "Date", "Result", "Home Team", "Away Team"]
    )
    inputs = inputs.to_numpy(dtype=np.float64)
    targets = (
        matches_df["Result"].replace({"H": 0, "D": 1, "A": 2}).to_numpy(dtype=np.int64)
    )
...
def construct_input_from_team_names(
    ...
    home_team_row = (
        matches_df[matches_df["Home Team"] == home_team]
        .head(1)
        .drop(columns=["Season", "Date", "Result", "Home Team", "Away Team"])
    )
    away_team_row = (
        matches_df[matches_df["Away Team"] == away_team]
        .head(1)
        .drop(columns=["Season", "Date", "Result", "Home Team", "Away Team"])
    )
    return np.hstack(
        (
            ...
            home_team_row[[col for col in home_team_row.columns if col[0] == "H"]]
            .to_numpy(dtype=np.float64)
            .flatten(),
            away_team_row[[col for col in home_team_row.columns if col[0] == "A"]]
            .to_numpy(dtype=np.float64)
            .flatten(),
        )
    ).reshape((1, -1))
