...
class MainApplicationWindow:
    ...
    def open(self):
        ...
        if not self._has_shown_welcome_message:
            messagebox.showinfo(
                "Welcome", "Thank you for using our product! Always Bet Responsibly :)"
            )
    ...
    def _add_menubar(self):
        ...
        analysis_menu.add_command(
            label="Feature Importance", command=self._plot_importance
        )
        analysis_menu.add_command(
            label="Target Distribution", command=self._plot_target_distribution
        )
        ...
        train_menu.add_command(
            label="Neural Network (Auto Tuning)", command=self._tune_nn
        )
        train_menu.add_command(
            label="Neural Network (Custom)", command=self._train_custom_nn
        )
        train_menu.add_separator()
        train_menu.add_command(
            label="Random Forest (Auto Tuning)", command=self._tune_rf
        )
        train_menu.add_command(
            label="Random Forest (Custom)", command=self._train_custom_rf
        )
        ...
        help_menu.add_checkbutton(
            label="Display Help", onvalue=1, offvalue=0, variable=self._show_hints_var
        )
        help_menu.add_command(
            label="About",
            command=lambda: webbrowser.open("https://kochlisgit.github.io/aboutme/"),
        )
    ...
    def _create_league(self):
        if self._matches_df is not None:
            messagebox.showwarning(
                "League is Open",
                'Another league is currently opened. Select "Close League"'
                "in order to be able to create another league",
            )
            return

        (
            self._open_league_name,
            self._open_league,
            self._matches_df,
        ) = CreateLeagueDialog(
            root=self._root, league_repository=self._league_repository
        ).open()
    ...
    def _load_league(self):
        ...
        if len(self._league_repository.get_all_saved_leagues()) == 0:
            messagebox.showwarning(
                "No Saved League",
                'No leagues have been created yet. To create one, click on "Create League" menu',
            )
            return
    ...
    def _delete_league(self):
        if len(self._league_repository.get_all_saved_leagues()) == 0:
            messagebox.showwarning(
                "No Saved League",
                "No leagues have been created yet, so this option is not available",
            )
            return
    ...
    def _evaluate_models(self):
        if (
            self._model_repository.get_all_models(league_name=self._open_league_name)
            is None
        ):
    ...
    def _change_theme(self):
        ...
        try:
            if theme_name == "winnative":
                ...
                self._root.tk.call(
                    "source", "database/storage/themes/forest/forest-light.tcl"
                )
                ttk.Style(self._root).theme_use("forest-light")
            elif theme_name == "forest-dark":
                self._root.tk.call(
                    "source", "database/storage/themes/forest/forest-dark.tcl"
                )
                ttk.Style(self._root).theme_use("forest-dark")
            elif theme_name == "breeze-light":
                self._root.tk.call(
                    "source", "database/storage/themes/breeze/breeze/breeze.tcl"
                )
                ttk.Style(self._root).theme_use("breeze-light")
            elif theme_name == "breeze-dark":
                self._root.tk.call(
                    "source",
                    "database/storage/themes/breeze/breeze-dark/breeze-dark.tcl",
                )
            ...
            print(
                "Exception Error: Cannot dynamically change theme more than once, due to TKinter Bug"
            )
