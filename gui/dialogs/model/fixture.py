...
class FixturesDialog(Dialog):
    def __init__(
        ...
        super().__init__(
            ...
            window_size={"width": 900, "height": 690},
        )
    ...
    def _initialize(self):
        Button(self.window, text="Import Fixture", command=self._import_fixture).place(
            x=20, y=20
        )
        ...
        months_cb = Combobox(
            self.window, state="readonly", width=10, textvariable=self._month_var
        )
        ...
        days_cb = Combobox(
            self.window, state="readonly", width=10, textvariable=self._day_var
        )
        days_cb["values"] = days
        days_cb.current(0)
        ...
        self._predict_btn = Button(
            ...
            state="disabled",
            command=self._submit_fixture_prediction,
        )
    ...
    def _edit_item_on_double_click_event(self, event):
        ...
        if column_name == "Home Team" or column_name == "Away Team":
            ...
            cb = Combobox(
                self.window, state="readonly", width=15, textvariable=team_var
            )
            ...
            if column_name != "Date":
                ...
                entry = Entry(
                    self.window, validate="key", validatecommand=(validate_float, "%P")
                )
            else:
