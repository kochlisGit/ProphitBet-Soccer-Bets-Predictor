...
class CreateLeagueDialog(Dialog):
    def __init__(self, root, league_repository: LeagueRepository):
        ...
        self._home_columns_vars = [IntVar(value=1) for _ in range(len(self._home_columns))]
        self._away_columns_vars = [IntVar(value=1) for _ in range(len(self._away_columns))]
    ...
    def _initialize(self):
        ...
        Label(self.window, text='Last N Matches Lookup', font=('Arial', 16)).place(x=20, y=90)
        Label(self.window, text='Goal Diff Margin', font=('Arial', 16)).place(x=20, y=130)
        Label(self.window, text='Statistics Columns', font=('Arial', 16)).place(x=170, y=180)
        ...
        goal_diff_entry = Entry(self.window, width=10, font=('Arial', 10), textvariable=self._goal_diff_margin_var)
        ...
        for i, col in enumerate(self._home_columns):
            Checkbutton(self.window, text=col, variable=self._home_columns_vars[i], onvalue=1, offvalue=0).place(
                x=40 + i*60, y=220
            )
        for i, col in enumerate(self._away_columns):
            Checkbutton(self.window, text=col, variable=self._away_columns_vars[i], onvalue=1, offvalue=0).place(
                x=40 + i*60, y=240
            )

        Button(self.window, text='Create', command=self._create_league).place(x=175, y=300)
        Button(self.window, text='Cancel', command=self.close).place(x=295, y=300)

        leagues_cb.bind(
            '<<ComboboxSelected>>',
            lambda event: self._league_selection_callback(event=event, league_name_entry=league_name_entry)
        )
    ...
    def _league_selection_callback(self, event, league_name_entry):
        ...
        league_name_entry.insert(
            0,
            self._selected_league_var.get().replace(' ', '').split('-', maxsplit=1)[1].replace('-', '')
        )
    ...
    def _create_league(self):
        ...
        if validation_result == 'valid':
            if not self._league_repository.league_exists(league_name=league_name):
                ...
                task_thread = threading.Thread(target=self._store_league, args=(task_dialog,))
                task_thread.start()
                task_dialog.open()
                self.close()
            else:
    ...
    def _store_league(self, task_dialog: TaskDialog):
        ...
        selected_home_columns = [col for i, col in enumerate(self._home_columns) if self._home_columns_vars[i].get()]
        selected_away_columns = [col for i, col in enumerate(self._away_columns) if self._home_columns_vars[i].get()]
...
class LoadLeagueDialog(Dialog):
    def __init__(self, root, league_repository: LeagueRepository):
        super().__init__(root=root, title='Load League', window_size={'width': 350, 'height': 160})
    ...
    def _initialize(self):
        Label(self.window, text='-- Select League --', font=('Arial', 16)).place(x=100, y=10)

        leagues_cb = Combobox(
            self.window, width=15, font=('Arial', 10), state='readonly', textvariable=self._league_name_var
        )
        ...
        Checkbutton(
            self.window, text='Update League', onvalue=True, offvalue=False, width=15, variable=self._update_var,
        ).place(x=130, y=80)

        Button(self.window, width=20, text='Load', command=self._load_league).place(x=110, y=125)

    def _load_league(self):
        task_dialog = TaskDialog(self.window, self._title)
        task_thread = threading.Thread(target=self._load_league_matches, args=(task_dialog,))
        task_thread.start()
        task_dialog.open()
        self.close()
...
class DeleteLeagueDialog(Dialog):
    def __init__(self, root, league_repository: LeagueRepository, open_league_name: str or None):
        super().__init__(root=root, title='Delete League', window_size={'width': 350, 'height': 120})
    ...
    def _initialize(self):
        ...
        Button(self.window, width=20, text='Delete', command=self._delete_league).place(x=30, y=80)
        Button(self.window, width=20, text='Close', command=self.close).place(x=200, y=80)

    def _delete_league(self):
        task_dialog = TaskDialog(self.window, self._title)
        task_thread = threading.Thread(target=self._delete_league_files, args=(task_dialog,))
        task_thread.start()
        task_dialog.open()
    ...
    def _delete_league_files(self, task_dialog: TaskDialog):
        ...
        if self._open_league_name is not None and league_name == self._open_league_name:
            ...
            messagebox.showinfo('League Deleted', f'League "{league_name}" has been successfully deleted')
